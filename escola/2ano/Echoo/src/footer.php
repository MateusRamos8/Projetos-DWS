...
<?php
        if(isset($_SESSION["result"])){
            if(isset($_SESSION["result_nulo"]) && $_SESSION["result_nulo"]){
                ?>
                <div class="notificacao bg-white border border-slate-300 max-w-[25vw] shadow-lg rounded-md p-4 flex justify-center fixed bottom-10 right-10 z-30 break-all break-words box-border">
                        <div class="flex gap-4">
                            ...
                            <section class="size-12 flex flex-col items-center justify-start">
                                <button type="button" onclick="document.querySelector('div.notificacao').remove()">
    ...
    ?> 
    <script>
    // Verificar se as variáveis de "result", "result_nulo", "titulo" e "msg" estão no sessionStorage
    window.onload = function() {
        if (sessionStorage.getItem("result")) {
            const result = sessionStorage.getItem("result");
            const result_nulo = sessionStorage.getItem("result_nulo");
            const titulo = sessionStorage.getItem("titulo");
            const msg = sessionStorage.getItem("msg");

            // Condicional para verificar o valor de "result_nulo"
            if (result_nulo && result_nulo === "true") {
                createNotification(titulo, msg, 'red');
                sessionStorage.removeItem("result_nulo"); // Limpa o valor após exibir a notificação
            } else if (result === "true") {
                createNotification(titulo, msg, 'green');
            } else {
                createNotification(titulo, msg, 'gray');
            }

            // Limpar as variáveis de sessão após a exibição da notificação
    ...
    // Função para criar a notificação dinamicamente
    function createNotification(titulo, msg, color) {
        const notification = document.createElement("div");
        notification.classList.add("notificacao", "bg-white", "border", "border-slate-300", "max-w-[25vw]", "shadow-lg", "rounded-md", "p-4", "flex", "justify-center", "fixed", "bottom-10", "right-10", "z-30", "break-all", "break-words", "box-border");
        
        notification.innerHTML = `
            <div class="flex gap-4">
                <section class="flex flex-col items-center justify-between gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-12 stroke-${color}">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.182 15.182a4.5 4.5 0 0 1-6.364 0M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0ZM9.75 9.75c0 .414-.168.75-.375.75S9 10.164 9 9.75 9.168 9 9.375 9s.375.336.375.75Zm-.375 0h.008v.015h-.008V9.75Zm5.625 0c0 .414-.168.75-.375.75s-.375-.336-.375-.75.168-.75.375-.75.375.336.375.75Zm-.375 0h.008v.015h-.008V9.75Z" />
                    </svg>

                    <div class="w-2 h-full bg-${color}-300 rounded-lg"> </div>
                </section>
                <section class="h-full flex flex-col items-start justify-end gap-2 pb-3">
                    <h1 class="text-base font-semibold text-zinc-800 antialiased">${titulo}</h1>
                    <p class="text-sm font-medium text-zinc-400 antialiased">${msg}</p>
                </section>
                <section class="size-12 flex flex-col items-center justify-start">
                    <button type="button" onclick="this.closest('div.notificacao').remove()">
                        <svg width="100%" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" class="cursor-pointer size-6">
                            <path d="M4.06585 3.00507C3.77296 2.71218 3.29809 2.71218 3.00519 3.00507C2.7123 3.29796 2.7123 3.77284 3.00519 4.06573L4.06585 3.00507ZM10.0763 11.1368C10.3692 11.4297 10.844 11.4297 11.1369 11.1368C11.4298 10.8439 11.4298 10.369 11.1369 10.0761L10.0763 11.1368ZM3.00519 4.06573L10.0763 11.1368L11.1369 10.0761L4.06585 3.00507L3.00519 4.06573Z" fill="#989fac" />
